...
public class LeaderboardController : MonoBehaviour
{
    ...
    string privateCode = "DM me if you want this";
    string publicCode = "DM me if you want this";
    ...
    public static LeaderboardController GetSceneDreamloLeaderboard()
    {
        ...
        return go.GetComponent<LeaderboardController>();
    }
    ...
    public void DeleteScore(string playerName, string playerName2, int totalScore)
    {
        if (TooManyRequests()) return;

        StartCoroutine(DeleteScoreWithPipe(playerName, playerName2, totalScore));
    }
    ...
    IEnumerator DeleteScoreWithPipe(string playerName, string playerName2, int totalScore)
    {
        playerName = Clean(playerName);

        WWW www = new WWW(dreamloWebserviceURL + privateCode + "/delete/" + WWW.EscapeURL(playerName));
        yield return www;
        StartCoroutine(AddScoreWithPipe(playerName2, totalScore));
    ...
    IEnumerator GetScores2()
    {
        highScores = "";
        WWW www = new WWW(dreamloWebserviceURL + publicCode + "/pipe");
        yield return www;
        highScores = www.text;
        obj.GetComponent<LB_StandardHighScores>().loading = false;
    }
    ...
    public void LoadScores(int x)
    {
        if (TooManyRequests()) return;
        StartCoroutine(GetScores2());
    }
