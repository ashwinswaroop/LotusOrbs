...
public class LB_Leaderboard : MonoBehaviour
{
    ...
    public int maxNameLength = 16; // <-- change this to allow longer names
    ...
    public string GetScoreAsFloatAt(int index)
    {
        // quick check to make sure that the leaderboard was set up before it is accessed
        if (!doneSetup)
        {
    ...
    public void LoadScores(List<LeaderboardController.Score> leaderboardScores)
    {
        // set up arrays to store scores in for processing
        names = new string[numberOfScores];
        timeScore = new string[numberOfScores];

        // load our scores out of player prefs
        for (var i = 0; i < numberOfScores; i++)
        {
            highScore = leaderboardScores[i].score;
            names[i] = leaderboardScores[i].playerName;
            timeScore[i] = highScore / 100 + ":" + highScore % 100;
        }
